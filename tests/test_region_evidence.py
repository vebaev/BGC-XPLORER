...
class RegionEvidenceTests(unittest.TestCase):
    ...
    def test_single_table_includes_all_rows_sortable_markers_and_safe_text(self):
        ...
        html = render_evidence_table(
            frame, {}, lambda row, index: "<button>Map</button>", min_containment=0.45
        )
        self.assertEqual(html.count("<tr data-search="), 2)
        self.assertIn("data-column='4' data-type='number'", html)
        self.assertIn(">Location</button>", html)
        self.assertIn(">Signals</button>", html)
        self.assertNotIn(">Product signals</button>", html)
        self.assertIn("<span class='arts-badge arts-known'>1 known</span>", html)
        self.assertIn("<span class='arts-badge arts-duf'>2 DUF</span>", html)
        self.assertIn("id='evidence-mibig'", html)
        self.assertIn("id='evidence-known'", html)
        self.assertIn("id='evidence-duf'", html)
        self.assertIn("<strong>Method</strong>: KnownClusterBlast", html)
        self.assertNotIn("Score metric", html)
        self.assertNotIn("Match score", html)
        self.assertIn("antiSMASH", html)
        self.assertIn("GECCO", html)
        self.assertIn("DeepBGC", html)
        self.assertIn("members cover \u226545% of the shorter region", html)
        self.assertIn("<dl class='column-legend'>", html)
        self.assertNotIn("<h2>Cluster evidence</h2>", html)
        self.assertNotIn("Core-gene evidence", html)
        self.assertNotIn("Edge distance (bp)", html)
