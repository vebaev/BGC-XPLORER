...
- Groups predictions into candidate loci by counting callers per gene rather than by comparing whole regions. Callers disagree about boundaries mostly by convention — antiSMASH extends past the core genes, GECCO cuts at CDS edges, DeepBGC at domains — so a locus is the run of genes at least two of the three callers place inside a cluster. A prediction no run covers is kept whole as a single-caller locus. The reported coordinates are the agreed span; `union_start`/`union_end` carry the full extent, and the gene tables and maps are built from it, so no gene any caller reported is dropped. Set `consensus.mode: "containment"` to restore the previous pairwise rule.
